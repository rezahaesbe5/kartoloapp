...
/** Halaman User Member produk Administration. */
export function UserMemberPage() {
  ...
  const query = useQuery({
    queryKey: ['admin.member.list', { page, pageSize, search, sortBy, sortDir }],
    queryFn: () => getUsers({ page, page_size: pageSize, search: search || undefined, sort_by: sortBy, sort_dir: sortDir, user_type: 'member' }),
  ...
  return (
    <section className="space-y-6 animate-fade-in-up">
      ...
      <div className="glass-card rounded-3xl p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative w-full sm:max-w-xs">
            ...
            <input type="search" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Cari username, email, atau nama..." aria-label="Cari user member" className="field field-with-icon !py-2.5 text-sm" />
          </div>
        </div>
          ...
          <table className="w-full min-w-[760px] border-collapse">
            ...
            <tbody>
              ...
              {!query.isLoading && query.isError && (
                <tr><td colSpan={7} className="px-4 py-12"><div className="flex items-center justify-center gap-2 text-red-600 dark:text-red-400"><AlertCircle className="h-5 w-5" /><span className="text-sm">Gagal memuat data member. Coba muat ulang halaman.</span></div></td></tr>
              )}
              {!query.isLoading && !query.isError && items.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-12"><p className="text-center text-sm text-slate-500 dark:text-slate-400">{search ? `Tidak ada member yang cocok dengan "${search}".` : 'Belum ada data member.'}</p></td></tr>
              )}
      ...
      {/* Modals */}
      <UserFormModal open={formOpen} mode={formMode} initialData={editData} onClose={() => setFormOpen(false)} onSuccess={handleFormSuccess} entityLabel="Member" userType="member" />
      <UserDetailModal open={detailId !== null} userId={detailId} onClose={() => setDetailId(null)} userType="member" entityLabel="Member" />
      <MappingRoleModal
