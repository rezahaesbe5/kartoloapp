...
import { MappingProdukModal } from '../components/MappingProdukModal';
...
interface SortHeaderProps {
  label: string;
  col: ProdukSortBy;
  activeCol: ProdukSortBy;
  dir: 'asc' | 'desc';
  onSort: (col: ProdukSortBy) => void;
}
...
/** Halaman Produk produk Administration. */
export function ProdukPage() {
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState<number>(10);
  const [searchInput, setSearchInput] = useState('');
  const [search, setSearch] = useState('');
  const [sortBy, setSortBy] = useState<ProdukSortBy>('nama_produk');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');

  // Form modal state
  const [formOpen, setFormOpen] = useState(false);
  const [formMode, setFormMode] = useState<'create' | 'edit'>('create');
  const [editData, setEditData] = useState<ProdukRow | null>(null);

  // Mapping modal state
  const [mappingOpen, setMappingOpen] = useState(false);
  const [mappingProduk, setMappingProduk] = useState<ProdukRow | null>(null);

  // Delete confirmation state
  const [deleteConfirm, setDeleteConfirm] = useState<{ open: boolean; produk: ProdukRow | null }>({ open: false, produk: null });
  ...
  const query = useQuery({
    queryKey: ['admin.produk.list', { page, pageSize, search, sortBy, sortDir }],
    queryFn: () => getProdukList({ page, page_size: pageSize, search: search || undefined, sort_by: sortBy, sort_dir: sortDir }),
  ...
  function toggleSort(col: ProdukSortBy) {
    if (sortBy === col) {
  ...
  function openEdit(row: ProdukRow) {
    setFormMode('edit');
    setEditData(row);
    setFormOpen(true);
  }

  function openMapping(row: ProdukRow) {
    setMappingProduk(row);
    setMappingOpen(true);
  }
  ...
  function handleMappingSuccess(message: string) {
    query.refetch();
    showResult('success', message);
  }

  function openDeleteConfirm(row: ProdukRow) {
    setDeleteConfirm({ open: true, produk: row });
  }

  async function handleDelete() {
    if (!deleteConfirm.produk) return;
    setDeleteLoading(true);
    try {
      const { message } = await deleteProduk(deleteConfirm.produk.id);
      setDeleteConfirm({ open: false, produk: null });
      showResult('success', message);
      query.refetch();
    } catch (err: unknown) {
      setDeleteConfirm({ open: false, produk: null });
      showResult('error', apiErrorMessage(err, 'Gagal menghapus produk. Coba lagi.'));
    } finally {
      setDeleteLoading(false);
    }
  }

  const totalCols = 5; // No + Aksi + Nama Produk + URL Produk + Status
  ...
  return (
    <section className="space-y-6 animate-fade-in-up">
      ...
      <div className="glass-card rounded-3xl p-5 sm:p-6 overflow-hidden">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            aria-label="Buka modal tambah produk"
          >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <div className="group relative inline-block">
              <button
                ...
                aria-label="Refresh data produk"
          ...
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              ...
              placeholder="Cari nama atau URL produk..."
          ...
          <table className="w-full min-w-[600px] border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700/70">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 w-32">
                  Aksi
                </th>
                <SortHeader label="Nama Produk" col="nama_produk" activeCol={sortBy} dir={sortDir} onSort={toggleSort} />
                <SortHeader label="URL Produk" col="url_produk" activeCol={sortBy} dir={sortDir} onSort={toggleSort} />
                <SortHeader label="Status" col="active_flag" activeCol={sortBy} dir={sortDir} onSort={toggleSort} />
              </tr>
            </thead>
            <tbody>
              {query.isLoading && (
                ...
                <tr>
                  <td colSpan={totalCols} className="px-4 py-12">
                    <div className="flex items-center justify-center gap-2 text-red-600 dark:text-red-400">
                      ...
                      <span className="text-sm">Gagal memuat data produk. Coba muat ulang halaman.</span>
                    </div>
                  </td>
                </tr>
              )}
                ...
                <tr>
                  <td colSpan={totalCols} className="px-4 py-12">
                    <p className="text-center text-sm text-slate-500 dark:text-slate-400">
                      {search ? `Tidak ada produk yang cocok dengan "${search}".` : 'Belum ada data produk.'}
                    </p>
                  </td>
                </tr>
              )}
                  ...
                  >
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <div className="group relative inline-block">
                          <button
                            ...
                            aria-label={`Edit produk ${row.nama_produk}`}
                        ...
                        <div className="group relative inline-block">
                          <button
                            ...
                            aria-label={`Hapus produk ${row.nama_produk}`}
                        ...
                        <div className="group relative inline-block">
                          <button
                            ...
                            aria-label={`Mapping role produk ${row.nama_produk}`}
                            className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-violet-100 text-violet-700 ring-1 ring-violet-200/70 transition-all duration-150 hover:bg-violet-600 hover:text-white hover:ring-violet-600 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:ring-offset-1 dark:bg-violet-500/15 dark:text-violet-300 dark:ring-violet-500/30 dark:hover:bg-violet-500 dark:hover:text-white dark:hover:ring-violet-400 dark:focus-visible:ring-offset-slate-900"
                          >
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm text-slate-500 dark:text-slate-400 font-mono">
                        {row.url_produk ?? <span className="italic text-slate-400">—</span>}
                      </span>
                    </td>
                    {/* Status */}
                    <td className="px-4 py-3">
                      {row.active_flag ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-500/15 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-200/70 dark:ring-emerald-500/30">
                          Aktif
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 rounded-full bg-red-50 dark:bg-red-500/15 px-2.5 py-1 text-xs font-semibold text-red-700 dark:text-red-300 ring-1 ring-red-200/70 dark:ring-red-500/30">
                          Nonaktif
                        </span>
                      )}
                    </td>
      ...
      {deleteConfirm.open && (
        <DeleteConfirmModal
          produk={deleteConfirm.produk!}
          loading={deleteLoading}
          onConfirm={handleDelete}
          onCancel={() => setDeleteConfirm({ open: false, produk: null })}
        />
      )}
      ...
      <ResultAlert
        ...
        onClose={() => setResultAlert((s) => ({ ...s, open: false }))}
      />
    </section>
  );
}
    ...
    >
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 dark:border-slate-700/70 bg-white dark:bg-slate-900 shadow-2xl animate-pop-in p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-base font-bold text-slate-800 dark:text-slate-50">Hapus Produk?</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              Yakin ingin menghapus produk &quot;{produk.nama_produk}&quot;? Tindakan ini tidak dapat dibatalkan.
            </p>
          </div>
        </div>
